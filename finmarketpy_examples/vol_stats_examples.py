...
from finmarketpy.curve.volatility.volstats import VolStats  # noqa: E402
...
if run_example == 1 or run_example == 0:
    ...
    realized_vol = (
        vol_stats.calculate_realized_vol(
            "GBPUSD",
            tenor_label="ON",
            freq="intraday",
            freq_min_mult=1,
    ...
    vrp = vol_stats.calculate_vol_risk_premium(
        "GBPUSD", tenor_label="ON", implied_vol=implied_vol, realized_vol=realized_vol
    )
...
if run_example == 2 or run_example == 0:
    ...
    intraday_spot_df = calc.resample_tick_data_ohlc(market.fetch_market(md_request), "GBPUSD", freq="1min")[
        "GBPUSD.close"
    ]
...
if run_example == 4 or run_example == 0:
    ...
    implied_vol_df = filter.filter_time_series_by_time_of_day_timezone(
        10, 0, implied_vol_df, timezone_of_snap="America/New_York"
    )
    ...
    implied_vol_addon = vol_stats.calculate_implied_vol_addon(
        asset, implied_vol=implied_vol_df, tenor_label="ON", adj_ON_friday=True
    ).dropna()

    vrp = vol_stats.calculate_vol_risk_premium(
        asset, tenor_label="ON", implied_vol=implied_vol_df, realized_vol=realized_vol, adj_ON_friday=True
    )
