...
@guvectorize(
    ["void(f8[:], f8[:], f8[:,:], f8[:,:], intp, f8[:])"],
    "(n),(n),(n,m),(n,m),()->(n)",
    cache=True,
    target="cpu",
    nopython=True,
)
def _forwards_interpolate_numba(
    spot_arr, spot_delivery_days_arr, quoted_delivery_days_arr, forwards_points_arr, no_of_tenors, out
):
    for i, delivery_day in enumerate(spot_delivery_days_arr):
        for j in range(no_of_tenors):
            ...
            # Eg. if we're in between 1W and 2W
                ...
                #                                delivery_day * (forwards_points_arr[i, j+1] - forwards_points_arr[i, j]) \  # noqa: E501
                ...
                out[i] = (
                    spot_arr[i]
                    + forward_points_per_day * delivery_day
                    + forwards_points_arr[i, j]
                    - (forward_points_per_day * quoted_delivery_days_arr[i, j])
                )
...
@guvectorize(
    ["void(f8[:], f8[:,:], f8[:,:], f8[:,:], f8, f8, intp, f8[:,:])"],
    "(n),(n,m),(n,m),(n,m),(),(),()->(n,m)",
    cache=True,
    target="cpu",
    nopython=True,
)
def _infer_base_currency_depo_numba(
    spot_arr, outright_forwards_arr, depo_arr, quoted_delivery_days_arr, base_conv, terms_conv, no_of_tenors, out
):
    for i in range(len(out)):
        for j in range(no_of_tenors):
            out[i, j] = (
                (
                    (1 + depo_arr[i, j] * (quoted_delivery_days_arr[i, j] / terms_conv))
                    / (outright_forwards_arr[i, j] / spot_arr[i])
                )
                - 1
...
def _infer_terms_currency_depo_numba(
    spot_arr, outright_forwards_arr, depo_arr, quoted_delivery_days_arr, base_conv, terms_conv, no_of_tenors, out
):
    for i in range(len(out)):
        for j in range(no_of_tenors):
            out[i, j] = (
                (outright_forwards_arr[i, j] / spot_arr[i])
                * (1 + depo_arr[i, j] * (quoted_delivery_days_arr[i, j] / base_conv))
                - 1
            ) / (quoted_delivery_days_arr[i, j] / terms_conv)


def _forwards_interpolate(
    spot_arr, spot_delivery_days_arr, quoted_delivery_days_arr, forwards_points_arr, no_of_tenors
):
...
class FXForwardsPricer(AbstractPricer):
    ...
    def __init__(self, market_df=None, quoted_delivery_df=None):  # noqa: D107
    ...
    def price_instrument(
        ...
        if quoted_delivery_df is None:
            quoted_delivery_df = self.generate_quoted_delivery(
                cross, market_df, quoted_delivery_df, fx_forwards_tenor_for_interpolation, cross
            )
        ...
        quoted_delivery_df, quoted_delivery_days_arr, forwards_points_arr, divisor = self._setup_forwards_calculation(
            cross, spot_date, market_df, quoted_delivery_df, fx_forwards_tenor_for_interpolation
        )

        interpolated_outright_forwards_arr = _forwards_interpolate_numba(
            spot_arr,
            spot_delivery_days_arr,
            quoted_delivery_days_arr,
            forwards_points_arr,
            len(fx_forwards_tenor_for_interpolation),
        )
        ...
        if return_as_df:
            ...
            interpolated_df[cross + "-interpolated-forward-points.close"] = (
                interpolated_outright_forwards_arr - spot_arr
            ) * divisor
    ...
    def get_forwards_divisor(self, currency):  # noqa: D102
    ...
    def _setup_forwards_calculation(self, cross, spot_date, market_df, quoted_delivery_df, fx_forwards_tenor):
        ...
        for i, tenor in enumerate(fx_forwards_tenor):
            quoted_delivery_days_arr[:, i] = (
                pd.DatetimeIndex(quoted_delivery_df[cross + tenor + ".delivery"]) - spot_date
            ).days

        return quoted_delivery_df, quoted_delivery_days_arr, forwards_points_arr, divisor

    def generate_quoted_delivery(self, cross, market_df, quoted_delivery_df, fx_forwards_tenor, cal):  # noqa: D102
        ...
        if quoted_delivery_df is None:
            ...
            for tenor in fx_forwards_tenor:
                quoted_delivery_df[cross + tenor + ".delivery"] = self._calendar.get_delivery_date_from_horizon_date(
