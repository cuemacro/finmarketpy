...
def _spot_index_numba(spot, time_diff, base_deposit, terms_deposit, base_daycount, terms_daycount, out):
    ...
    for i in range(1, len(out)):
        ...
        out[i] = out[i - 1] * (
            1
            + (1 + base_deposit[i] * time_diff[i] / base_daycount) * (spot[i] / spot[i - 1])
        ...
        if construct_via_currency == "no":
            ...
            for tick in md_request.tickers:
                ...
                base_vals = self.fetch_continuous_time_series(
                    md_request_base, market_data_generator, construct_via_currency="no", field=field
                )
                terms_vals = self.fetch_continuous_time_series(
                    md_request_terms, market_data_generator, construct_via_currency="no", field=field
                )
