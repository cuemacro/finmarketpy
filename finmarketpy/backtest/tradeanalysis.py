...
class TradeAnalysis(object):
    ...
    def run_strategy_returns_stats(self,
        ...
        if engine == "pyfolio":
            # PyFolio assumes UTC time based DataFrames (so force this
            # localisation)
            try:
    ...
    def run_excel_trade_report(self, trading_model, excel_file="model.xlsx"):
        ...
        # TODO Add summary sheet comparing return statistics for all the
        # different models in the list
    ...
    def save_positions_trades(self, tm, signals, trades, signal_caption,
        ...
        recent_signals = tm._grab_signals(signals,
                                          date=[-1, -2, -5, -10, -20],
                                          strip=strip)
        recent_trades = tm._grab_signals(trades,
                                         date=[-1, -2, -5, -10, -20],
                                         strip=strip)

        recent_signals.to_excel(
            writer,
            sheet_name=tm.FINAL_STRATEGY + " " + signal_caption,
            engine="xlsxwriter",
        )
        recent_trades.to_excel(
            writer,
            sheet_name=tm.FINAL_STRATEGY + " " + trade_caption,
            engine="xlsxwriter",
        )

    def run_tc_shock(self,
                     strategy,
                     tc=None,
    ...
    def run_arbitrary_sensitivity(
        ...
        if not (reload_market_data):
            (
                ...
                basket_dict,
                contract_value_df,
            ) = self._load_assets(trading_model)
        ...
        if (market_constants.backtest_thread_no[market_constants.generic_plat] >
            ...
            for i in range(0, len(parameter_list)):
                ...
                for k in list(current_parameter.keys()):
                    setattr(br, k, current_parameter[k])
                    setattr(br.tech_params, k, current_parameter[k])

                # should specify reloading the data, if our parameters impact
                # which assets we are fetching
                if reload_market_data:
                    (
                        ...
                        basket_dict,
                        contract_value_df,
                    ) = self._load_assets(trading_model, br=br)
        ...
        style.file_output = (self.DUMP_PATH + trading_model.FINAL_STRATEGY +
                             " " + parameter_type + ".png")
        style.html_file_output = (self.DUMP_PATH +
        ...
        style = Style()
        style.file_output = (self.DUMP_PATH + trading_model.FINAL_STRATEGY +
                             " " + parameter_type + " IR.png")
        style.html_file_output = (self.DUMP_PATH +
                                  trading_model.FINAL_STRATEGY + " " +
                                  parameter_type + " IR.html")
        style.scale_factor = trading_model.SCALE_FACTOR
        style.title = trading_model.FINAL_STRATEGY + " " + parameter_type
        ...
        # plot all the rets
        style.file_output = (self.DUMP_PATH + trading_model.FINAL_STRATEGY +
                             " " + parameter_type + " Rets.png")
        style.html_file_output = (self.DUMP_PATH +
                                  trading_model.FINAL_STRATEGY + " " +
                                  parameter_type + " Rets.html")

        summary_rets = (pandas.DataFrame(
            index=pretty_portfolio_names, data=rets, columns=["Rets (%)"]) *
                        100)
    ...
    def _run_strategy(
        ...
        signal_df = trading_model.construct_signal(spot_df,
                                                   spot_df2,
                                                   br.tech_params,
                                                   br,
                                                   run_in_parallel=False)
    ...
    def run_day_of_month_analysis(self, trading_model, resample_freq="B"):
        ...
        bus_day = seas.bus_day_of_month_seasonality(rets,
                                                    add_average=True,
        ...
        style.scale_factor = trading_model.SCALE_FACTOR
        style.file_output = (self.DUMP_PATH + trading_model.FINAL_STRATEGY +
                             " seasonality month of year.png")
        style.html_file_output = (self.DUMP_PATH +
                                  trading_model.FINAL_STRATEGY +
                                  " seasonality month of year.html")
