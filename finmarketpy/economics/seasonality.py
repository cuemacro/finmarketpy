...
class Seasonality(object):
    ...
    def bus_day_of_month_seasonality(
        ...
        if cum is True:
            if partition_by_month:
                ...
                # monthly_seasonality.index = monthly_seasonality.index + 1
                # # shifting index
    ...
    def monthly_seasonality_from_prices(self,
                                        data_frame,
                                        cum=True,
                                        add_average=False):
        return self.monthly_seasonality(data_frame,
                                        cum,
                                        add_average,
                                        price_index=True)
    ...
    def adjust_rolling_seasonality(self,
                                   data_frame,
                                   window=None,
                                   likely_period=None):
        ...
        for c in data_frame.columns:
            data_frame[c] = (data_frame[c].rolling(
                center=False, window=window,
                min_periods=window).apply(lambda x: self._remove_seasonality(
                    x, likely_period=likely_period)))
