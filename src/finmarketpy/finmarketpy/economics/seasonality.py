...
class Seasonality:
    ...
    def time_of_day_seasonality(self, data_frame, years=False, seconds=False):  # noqa: D102
    ...
    def bus_day_of_month_seasonality_from_prices(  # noqa: D102
        self,
        data_frame,
        month_list=None,
        cum=True,
    ...
    def bus_day_of_month_seasonality(  # noqa: D102
        self,
        data_frame,
        month_list=None,
        cum=True,
        ...
        if cum is True:
            if partition_by_month:
                monthly_seasonality.loc[0] = np.zeros(len(monthly_seasonality.columns))
    ...
    def monthly_seasonality_from_prices(self, data_frame, cum=True, add_average=False):  # noqa: D102
        return self.monthly_seasonality(data_frame, cum, add_average, price_index=True)

    def monthly_seasonality(self, data_frame, cum=True, add_average=False, price_index=False):  # noqa: D102

        calculations = Calculations()

        if price_index:
            data_frame = data_frame.resample("BME").mean()  # resample into month end
            data_frame = calculations.calculate_returns(data_frame)
    ...
    def adjust_rolling_seasonality(self, data_frame, window=None, likely_period=None):
        ...
        for c in data_frame.columns:
            data_frame[c] = (
                data_frame[c]
                .rolling(center=False, window=window, min_periods=window)
                .apply(lambda x: self._remove_seasonality(x, likely_period=likely_period))
            )
    ...
    def _remove_seasonality(self, series, likely_period=None):
        ...
        seasons, _trend = fit_seasons(series, period=likely_period)
