...
def test_sma():
    ...
    col_prices = np.array(list(range(1, 9)))
    data_df = pd.DataFrame(index=dates,
                           columns=cols,
                           data=np.tile(col_prices, (len(cols), 1)).T)
    tech_ind.create_tech_ind(data_df, indicator_name, tech_params)
    expected_signal_df = pd.DataFrame(
        index=dates,
        columns=[" ".join([col, indicator_name, "Signal"]) for col in cols],
    ...
    cols = get_cols_name(10)
    col_prices = np.array(list(range(8, 0, -1)))
    data_df = pd.DataFrame(index=dates,
                           columns=cols,
                           data=np.tile(col_prices, (len(cols), 1)).T)
    tech_ind.create_tech_ind(data_df, indicator_name, tech_params)
...
def test_polarity():
    ...
    expected_signal_df = pd.DataFrame(
        index=dates,
        columns=[" ".join([col, "Polarity", "Signal"]) for col in cols],
        data=1,
    )
    expected_df = pd.DataFrame(
        index=dates,
...
def test_attr():
    ...
    # Test Case 2: Check incompatibility between only_allow_longs and
    # only_allow_shorts
