            ...
            if cross[0:3] == cross[3:6]:
                ...
                delivery_date.append(
                    self._calendar.get_delivery_date_from_horizon_date(
                        horizon_date[0], fx_forwards_trading_tenor, cal=cross, asset_class="fx"
                    )[0]
                )
                ...
                for i in range(1, len(horizon_date)):
                    ...
                    if new_trade[i]:
                        delivery_date.append(
                            self._calendar.get_delivery_date_from_horizon_date(
                                horizon_date[i], fx_forwards_trading_tenor, cal=cross, asset_class="fx"
                            )[0]
                        )
                ...
                interpolated_forward = fx_forwards_pricer.price_instrument(
                    cross,
                    horizon_date,
                    delivery_date,
                ...
                for i in range(1, len(horizon_date)):
                    if new_trade[i]:
                        mtm[i] = fx_forwards_pricer.price_instrument(
                            cross,
                            horizon_date[i],
                            delivery_date[i - 1],
                            market_df=forwards_market_df,
