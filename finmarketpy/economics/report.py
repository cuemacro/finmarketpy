...
class Report(object):
    ...
    def report_single_var_regression(self, y, x, y_variable_names,
                                     x_variable_names, statistic,
                                     pretty_index=None):
        ...
        stats = Calculations.linear_regression_single_vars(y, x,
                                                           y_variable_names,
                                                           x_variable_names)
        ...
        stats_df = Calculations.strip_linear_regression_output(pretty_index,
                                                               stats,
                                                               statistic)
    ...
    def plot_single_var_regression(self, y, x, y_variable_names,
                                   x_variable_names, statistic,
                                   tag='stats',
                                   title=None,
        ...
        for st in statistic:
            ...
            for sh in shift:
                x_sh = x.shift(sh)
                stats_temp = self.report_single_var_regression(y, x_sh,
                                                               y_variable_names,
                                                               x_variable_names,
                                                               st,
                                                               pretty_index)

                stats_temp.columns = [x + "_" + str(sh) for x in
                                      stats_temp.columns]
