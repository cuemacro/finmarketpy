...
class FXOptionsCurve:
    ...
    def fetch_continuous_time_series(  # noqa: D102
        self,
        md_request,
        ...
        # Eg. we construct EURJPY via EURJPY directly (note: would need to have sufficient options/forward data for this)  # noqa: E501
    ...
    def unhedged_asset_fx(self, assets_df, asset_currency, home_curr, start_date, finish_date, spot_df=None):  # noqa: D102
        pass

    def hedged_asset_fx(  # noqa: D102
        self, assets_df, asset_currency, home_curr, start_date, finish_date, spot_df=None, total_return_indices_df=None
    ):
        pass

    def get_day_count_conv(self, currency):  # noqa: D102
        if currency in market_constants.currencies_with_365_basis:
            return 365.0

        return 360.0

    def construct_total_return_index(  # noqa: D102
        self,
        cross_fx,
        market_df,
        fx_vol_surface=None,
        ...
        for cross in cross_fx:
            ...
            if cross[0:3] == cross[3:6]:
                ...
                if enter_trading_dates is None:
                    ...
                    expiry_date[0] = self._calendar.get_expiry_date_from_horizon_date(
                        pd.DatetimeIndex([horizon_date[0]]), fx_options_trading_tenor, cal=cal, asset_class="fx-vol"
                    )[0]
                    ...
                    for i in range(1, len(horizon_date)):
                        ...
                        # If we're entering a new trade/contract (and exiting an old trade) we need to get new expiry and roll dates  # noqa: E501
                        if new_trade[i]:
                            ...
                            if horizon_date[i] <= expiry_date[i - 1]:
                                # Otherwise use previous expiry and roll dates, because we're still holding same contract  # noqa: E501
                    ...
                    for i in range(0, len(horizon_date)):
                        ...
                        if new_trade[i]:
                            ...
                            if i > 0:
                                # Check there's valid expiry on previous day (if not then we're not in an option trade here!)  # noqa: E501
                ...
                for i in range(1, len(horizon_date)):
                    if exit_trade[i]:
                        ...
                        calculated_strike[i] = calculated_strike[
                            i - 1
                        ]  # Note: this will get overwritten if there's a new trade
                    ...
                    if new_trade[i]:
                        ...
                        option_values_, spot_, strike_, vol_, delta_, expiry_date_, intrinsic_values_ = (  # noqa: RUF059
                            fx_options_pricer.price_instrument(
                                cross,
                                horizon_date[i],
                                calculated_strike[i],
    ...
    def apply_tc_signals_to_total_return_index(  # noqa: D102
        ...
        for cross in cross_fx:
            ...
            # q = abs(total_return_index_df[cross + '-delta.close'] - total_return_index_df[cross + '-delta.close'].shift(1)) * spot_tc  # noqa: E501

            # Additional columns to include P&L with transaction costs
            total_return_index_df[cross + "-option-return-with-tc.close"] = (
                total_return_index_df[cross + "-option-return.close"]
                - abs(total_return_index_df[cross + "-new-trade.close"].shift(1)) * option_tc
            )
            total_return_index_df[cross + "-delta-pnl-return-with-tc.close"] = (
                total_return_index_df[cross + "-delta-pnl-return.close"]
                - abs(
                    total_return_index_df[cross + "-delta.close"]
                    - total_return_index_df[cross + "-delta.close"].shift(1)
                )
                * spot_tc
            )

            total_return_index_df.iloc[
                0, total_return_index_df.columns.get_loc(cross + "-option-return-with-tc.close")
            ] = 0
            total_return_index_df.iloc[
                0, total_return_index_df.columns.get_loc(cross + "-delta-pnl-return-with-tc.close")
            ] = 0
            ...
            if cum_index == "mult":
                ...
                cum_delta_rets = 100 * np.cumprod(
                    1.0 + total_return_index_df[cross + "-delta-pnl-return-with-tc.close"].values
                )
