"""Backtest engine for computing trading strategy PnL, signals, leverage, and portfolio analytics."""
...
import numpy as np
import pandas as pd
from findatapy.util import LoggerManager, SwimPool
...
class Backtest:
    """Conducts backtest for strategies trading assets.

    Assumes we have an input of total returns. Reports historical return statistics
    and returns time series.
    """

    def __init__(self):
        """Initialise Backtest with empty PnL and portfolio attributes."""
        self._pnl = None
        self._portfolio = None
        return

    def calculate_diagnostic_trading_PnL(  # noqa: N802
        self,
        ...
        """
        if further_df_labels is None:
            further_df_labels = []
        if further_df is None:
            further_df = []
    ...
    def calculate_trading_PnL(  # noqa: N802
        self,
        br: BacktestRequest,
        asset_a_df: pd.DataFrame,
        signal_df: pd.DataFrame,
        ...
        """Calculate P&L of a trading strategy and statistics to be retrieved later.
        ...
        if br.take_profit is not None and br.stop_loss is not None:
            ...
            #
            #     to_plot = _calculations.pandas_outer_join(
            #         [asset_df_copy, pre_signal_df, signal_df_copy, trade_rets_df_copy, temp_strategy_rets_df]
            #     )
        ...
        self._components_pnl = self._filter_by_plot_start_finish_date(
            calculations.calculate_signal_returns_with_tc_matrix(
                portfolio_signal_before_weighting, returns_df, tc=tc, rc=rc
            ),
            br,
        )
        ...
        if br.portfolio_notional_size is not None:
            ...
            self._portfolio_signal_trade_notional = (
                self._portfolio_signal_notional - self._portfolio_signal_notional.shift(1)
            )
            ...
            if contract_value_df is not None:
                ...
                self._portfolio_signal_trade_contracts = (
                    self._portfolio_signal_contracts - self._portfolio_signal_contracts.shift(1)
                )
        ...
        if False:  # market_constants.backtest_thread_no[market_constants.generic_plat] > 1 and run_in_parallel:
            ...
            results_a = pool.apply_async(calculations.create_mult_index, args=(self._pnl,))
            results_b = pool.apply_async(calculations.create_mult_index, args=(self._components_pnl,))
            results_c = pool.apply_async(calculations.create_mult_index, args=(self._portfolio,))
            ...
            self._pnl_cum = results_a.get()
            self._components_pnl_cum = results_b.get()
            self._portfolio_cum = results_c.get()
            ...
            if br.cum_index == "mult":
                ...
                # Calculate individual signals cumulative P&L after signal leverage
                # but before portfolio level constraints
                self._pnl_cum = calculations.create_add_index(self._pnl)

                # Calculate individual signals cumulative P&L after signal leverage
                # AND after portfolio level constraints
    ...
    def _filter_by_plot_start_finish_date(self, df: pd.DataFrame, br: BacktestRequest) -> pd.DataFrame:
    ...
    def calculate_exposures(self, portfolio_signal: pd.DataFrame) -> list[pd.DataFrame]:
        """Calculate time series for the total longs, short, net and absolute exposure on an aggregated portfolio basis.
    ...
    def pnl(self) -> pd.DataFrame:
        """Get P&L returns of all individual sub-components of the model before any portfolio level leverage is applied.
    ...
    def components_pnl(self) -> pd.DataFrame:
        """Get P&L returns of all individual subcomponents of the model (after portfolio level leverage is applied).
    ...
    def components_pnl_trades(self) -> pd.DataFrame:
        ...
        if self._components_pnl_trades is None:
            ...
            self._components_pnl_trades = calculations.calculate_individual_trade_gains(
                self._signal, self._components_pnl
            )
    ...
    def portfolio_signal(self) -> pd.DataFrame:
        """Get the signals (with individual leverage and portfolio leverage) for each asset.
    ...
    def portfolio_signal_notional(self) -> pd.DataFrame:
        """Get signals (with leverage) for each asset scaled by a specified notional amount.
...
from chartpy import Chart, ChartConstants, Style  # noqa: E402
from findatapy.timeseries import Calculations, Filter, RetStats  # noqa: E402

from finmarketpy.economics import TechParams  # noqa: E402


class TradingModel:
    """Abstract class which wraps around Backtest, providing convenient functions for analysis.

    Implement your own subclasses of this for your own strategy. See tradingmodelfxtrend_example.py
    for a simple implementation of a FX trend following strategy.
    """
    ...
    def __init__(self):
        """Initialise TradingModel by loading backtest parameters."""
    ...
    def load_assets(self, br=None):
        """Load time series for the assets to be traded and for signal generation.
    ...
    def load_model(path):
        ...
        """
        pkl = pickle.load(path)  # noqa: S301
        return pkl
    ...
    def construct_strategy(self, br: BacktestRequest = None, run_in_parallel: bool = False) -> pd.DataFrame:
        ...
        if isinstance(market_data, (tuple, list)):
            ...
            spot_df = market_data["spot_df"] if "spot_df" in market_data else market_data["asset_df"]
        ...
        if market_constants.backtest_thread_no[market_constants.generic_plat] > 1 and run_in_parallel:
            ...
            for key in bask_keys:
                if key != self.FINAL_STRATEGY:
                    ...
                    mult_results.append(
                        pool.apply_async(
                            self.construct_individual_strategy,
                            args=(
                                br,
                                spot_cut_df,
                                spot_df2,
    ...
    def _assign_final_strategy_results(self, results: pd.DataFrame, backtest: Backtest):
        ...
        self._strategy_components_pnl = (
            backtest.components_pnl_cum()
        )  # P&L of the individual components (after portfolio level vol adjustments)
        self._strategy_components_pnl_ret_stats = (
            backtest.components_pnl_ret_stats().split_into_dict()
        )  # backtest.get_pnl_components_ret_stats()
        ...
        self._strategy_group_pnl_trades = (
            backtest.pnl_trades()
        )  # get individual trades P&L before (before portfolio adjustment)
    ...
    def construct_individual_strategy(
        ...
        signal = self.construct_signal(
            spot_df=spot_df, spot_df2=spot_df2, tech_params=tech_params, br=br, run_in_parallel=run_in_parallel
        )
        ...
        if key != self.FINAL_STRATEGY:
            ...
            return (
                desc,
                backtest.portfolio_cum(),
                backtest.portfolio_leverage(),
                backtest.portfolio_pnl_ret_stats(),
                key,
                None,
            )
    ...
    def compare_strategy_vs_benchmark(self, br: BacktestRequest, strategy_df: pd.DataFrame, benchmark_df: pd.DataFrame):
    ...
    def _flatten_list(self, list_of_lists: list[str]) -> list[str]:
        """Flatten list, particularly useful for combining baskets.
    ...
    def strategy_group_pnl_trades(self) -> pd.DataFrame:
        """Return the PnL for individual trades across all strategy groups."""
    ...
    def strategy_group_pnl(self) -> pd.DataFrame:
        """Return the cumulative PnL for all strategy groups."""
    ...
    def strategy_group_benchmark_pnl(self) -> pd.DataFrame:
        """Return the cumulative PnL for all strategy groups including benchmark."""
    ...
    def _reduce_plot(self, data_frame: pd.DataFrame, reduce_plot: bool = True, resample: str = "B") -> pd.DataFrame:
    ...
    def _chart_return_with_df(
        ...
        split_on_char: str | None = None,
    ):
        """Plot a DataFrame and optionally return it along with the chart."""
    ...
    def plot_individual_leverage(
        ...
        """Plot the individual leverage for each asset signal over time."""
        style = self._create_style("Individual Leverage", "Individual Leverage", reduce_plot=reduce_plot)

        with contextlib.suppress(Exception):
            df = self._strip_dataframe(
                self._reduce_plot(self._individual_leverage, reduce_plot=reduce_plot, resample=resample), strip
            )
    ...
    def plot_strategy_group_pnl_trades(
        ...
        """Plot individual trade PnL in basis points for the strategy group."""
        ...
        with contextlib.suppress(Exception):
            ...
            df = self._strip_dataframe(
                self._reduce_plot(strategy_pnl_trades, reduce_plot=reduce_plot, resample=resample), strip
            )
    ...
    def plot_strategy_pnl(
        ...
        """Plot the cumulative PnL for the final strategy."""
        style = self._create_style("", "Strategy PnL", reduce_plot=reduce_plot)

        try:
            df = self._strip_dataframe(
                self._reduce_plot(self._strategy_pnl, reduce_plot=reduce_plot, resample=resample), strip
            )
    ...
    def plot_strategy_trade_no(
        ...
        """Plot the number of trades for each signal in the strategy."""
    ...
    def plot_strategy_signal_proportion(
        ...
        """Plot the proportion of long, short, and flat signals for each asset."""
    ...
    def plot_strategy_leverage(
        ...
        """Plot the portfolio leverage of the final strategy over time."""
        style = self._create_style("Portfolio Leverage", "Strategy Leverage", reduce_plot=reduce_plot)

        with contextlib.suppress(Exception):
            df = self._strip_dataframe(
                self._reduce_plot(self._strategy_leverage, reduce_plot=reduce_plot, resample=resample), strip
            )
        ...
        ##### Plot the individual components cumulative returns and return statistics
        ##### (including portfolio level constraints)
    ...
    def plot_strategy_components_pnl(
        ...
        """Plot the cumulative PnL for each strategy component."""
        style = self._create_style("Ind Components", "Strategy PnL Components", reduce_plot=reduce_plot)

        with contextlib.suppress(Exception):
            df = self._strip_dataframe(
                self._reduce_plot(self._strategy_components_pnl, reduce_plot=reduce_plot, resample=resample), strip
            )
    ...
    def plot_strategy_components_pnl_ir(
        ...
        """Plot the information ratio for each strategy component."""
    ...
    def plot_strategy_components_pnl_returns(
        ...
        """Plot annualised returns for each strategy component."""
        return self._plot_ret_stats_helper(
            self._strategy_components_pnl_ret_stats,
            "Returns",
            "Ind Component (%)",
            "Ind Component Returns",
            strip=strip,
            silent_plot=silent_plot,
            ret_with_df=ret_with_df,
            split_on_char=split_on_char,
    ...
    def plot_strategy_components_pnl_vol(
        ...
        """Plot annualised volatility for each strategy component."""
        return self._plot_ret_stats_helper(
            self._strategy_components_pnl_ret_stats,
            "Vol",
            "Ind Component (%)",
    ...
    def plot_strategy_components_pnl_drawdowns(
        ...
        """Plot maximum drawdowns for each strategy component."""
        return self._plot_ret_stats_helper(
            self._strategy_components_pnl_ret_stats,
            "Drawdowns",
            "Ind Component (%)",
            "Ind Component Drawdowns",
            strip=strip,
            silent_plot=silent_plot,
            ret_with_df=ret_with_df,
            split_on_char=split_on_char,
    ...
    def plot_strategy_components_pnl_yoy(
        ...
        """Plot year-on-year returns for each strategy component."""
        ...
        ##### this will plot the final strategy, benchmark and all the individual baskets
        ##### (as though they were run by themselves)
    ...
    def plot_strategy_group_benchmark_pnl(
        ...
        """Plot the cumulative PnL for all strategy groups including the benchmark."""
        logger = LoggerManager().getLogger(__name__)
        ...
        df = self._strip_dataframe(
            self._reduce_plot(self._strategy_group_benchmark_pnl, reduce_plot=reduce_plot, resample=resample), strip
        )
    ...
    def plot_strategy_group_benchmark_pnl_ir(
        ...
        """Plot the information ratio for all strategy groups including the benchmark."""
        return self._plot_ret_stats_helper(
    ...
    def plot_strategy_group_benchmark_pnl_returns(
        ...
        """Plot annualised returns for all strategy groups including the benchmark."""
        return self._plot_ret_stats_helper(
            self._strategy_group_benchmark_pnl_ret_stats,
            "Returns",
            "(%)",
    ...
    def plot_strategy_group_benchmark_pnl_vol(
        ...
        """Plot annualised volatility for all strategy groups including the benchmark."""
        return self._plot_ret_stats_helper(
            self._strategy_group_benchmark_pnl_ret_stats,
            "Vol",
            "(%)",
    ...
    def plot_strategy_group_benchmark_pnl_drawdowns(
        ...
        """Plot maximum drawdowns for all strategy groups including the benchmark."""
        return self._plot_ret_stats_helper(
            self._strategy_group_benchmark_pnl_ret_stats,
            "Drawdowns",
            "(%)",
            "Group Benchmark Drawdowns",
            strip=strip,
            silent_plot=silent_plot,
            ret_with_df=ret_with_df,
            split_on_char=split_on_char,
    ...
    def _plot_ret_stats_helper(
        self, ret_stats, metric, title, file_tag, strip=None, silent_plot=False, ret_with_df=False, split_on_char=None
    ):
        ...
        return self._chart_return_with_df(
            ret_stats, style, silent_plot, chart_type="bar", ret_with_df=ret_with_df, split_on_char=split_on_char
        )
    ...
    def plot_strategy_group_benchmark_pnl_yoy(
        ...
        """Plot year-on-year returns for all strategy groups including the benchmark."""
        return self.plot_yoy_helper(
            self._strategy_group_benchmark_pnl_ret_stats,
            "",
    ...
    def plot_yoy_helper(
        ...
        """Plot year-on-year returns for the given return statistics dictionary."""
        style = self._create_style(title, title)
        # keys = self._strategy_group_benchmark_ret_stats.keys()
        yoy = []

        for key in ret_stats:
        ...
        return self._chart_return_with_df(
            ret_stats, style, silent_plot, chart_type="line", ret_with_df=ret_with_df, split_on_char=split_on_char
        )
    ...
    def plot_strategy_group_leverage(
        ...
        """Plot the leverage time series for all strategy groups."""
    ...
    def plot_strategy_all_signals(
        ...
        """Plot all position signals for the strategy as a time series."""
        ...
        if signal_show is not None and signal_show != []:
            not_found = []
    ...
    def plot_strategy_signals(
        ...
        """Plot position signals for the strategy on a specified date."""
    ...
    def plot_strategy_trades(
        ...
        """Plot trade signals (changes in position) for the strategy on a specified date."""
        return self._plot_signal(
    ...
    def plot_strategy_signals_notional(
        ...
        """Plot position signals scaled by the predefined notional amount."""
        return self._plot_signal(
    ...
    def plot_strategy_trades_notional(
        ...
        """Plot trade signals scaled by the predefined notional amount."""
        return self._plot_signal(
            self._strategy_trade_notional,
            label="trades (scaled by notional)",
            caption="Trades",
            date=date,
            strip=strip,
            silent_plot=silent_plot,
    ...
    def plot_strategy_trades_notional_sizes(
        ...
        """Plot the frequency distribution of trade sizes scaled by notional."""
        if strip is not None:
            self._strategy_trade_notional_sizes.index = [
                k.replace(strip, "") for k in self._strategy_trade_notional_sizes.index
            ]
    ...
    def plot_strategy_signals_contracts(
        ...
        """Plot position signals expressed in contract sizes (e.g. for futures)."""
        return self._plot_signal(
            self._strategy_signal_contracts,
    ...
    def plot_strategy_trades_contracts(
        ...
        """Plot trade signals expressed in contract sizes (e.g. for futures)."""
        return self._plot_signal(
            self._strategy_trade_contracts,
    ...
    def plot_strategy_total_exposures(
        ...
        """Plot total long, short, and absolute exposures of the strategy over time."""
    ...
    def plot_strategy_net_exposures(
        ...
        """Plot the net exposure (longs minus shorts) of the strategy over time."""
        style = self._create_style("", "Strategy Net Exposures", reduce_plot=reduce_plot)

        with contextlib.suppress(Exception):
            df = self._strip_dataframe(
                self._reduce_plot(self._strategy_net_exposure, reduce_plot=reduce_plot, resample=resample), strip
            )
            return self._chart_return_with_df(
                df, style, silent_plot, chart_type="line", ret_with_df=ret_with_df, split_on_char=split_on_char
            )
    ...
    def plot_strategy_total_exposures_notional(
        ...
        """Plot total long, short, and absolute exposures scaled by notional (in mm)."""
        style = self._create_style("(mm)", "Strategy Total Exposures (mm)", reduce_plot=reduce_plot)
        df = pd.concat(
            [
                self._strategy_total_longs_notional,
                self._strategy_total_shorts_notional,
                self._strategy_total_exposure_notional,
            ],
            axis=1,
        )
        ...
        with contextlib.suppress(Exception):
            df = self._strip_dataframe(
                self._reduce_plot(df / 1000000.0, reduce_plot=reduce_plot, resample=resample), strip
            )
            return self._chart_return_with_df(
                df, style, silent_plot, chart_type="line", ret_with_df=ret_with_df, split_on_char=split_on_char
            )
    ...
    def plot_strategy_net_exposures_notional(
        ...
        """Plot the net exposure of the strategy scaled by notional (in mm)."""
        style = self._create_style("(mm)", "Strategy Net Exposures (mm)", reduce_plot=reduce_plot)

        with contextlib.suppress(Exception):
            df = self._strip_dataframe(
                self._reduce_plot(
                    self._strategy_net_exposure_notional / 1000000.0, reduce_plot=reduce_plot, resample=resample
                ),
                strip,
            )
            return self._chart_return_with_df(
    ...
    def _plot_signal(
        ...
        return self._chart_return_with_df(
            last_day, style, silent_plot, chart_type="bar", ret_with_df=ret_with_df, split_on_char=split_on_char
        )
...
class PortfolioWeightConstruction:
    ...
    def __init__(self, br=None):
        """Initialise the portfolio weight construction with an optional BacktestRequest."""
        self._br = br
    ...
    def optimize_portfolio_weights(
        ...
        if br.signal_vol_adjust is True:
            leverage_df = self._risk_engine.calculate_leverage_factor(
                ...
                br.signal_vol_periods,
                br.signal_vol_obs_in_year,
        ...
        portfolio_signal = pd.DataFrame(
            data=np.multiply(leverage_matrix, signal_df.values), index=signal_df.index, columns=signal_df.columns
        )
        ...
        if br.portfolio_combination is not None:
            if "sum" in br.portfolio_combination:
                pass
            elif br.portfolio_combination == "mean" or (
                br.portfolio_combination == "weighted" and isinstance(br.portfolio_combination_weights, dict)
            ):
                portfolio_signal = pd.DataFrame(
    ...
    def calculate_signal_weights_for_portfolio(
        self, br: Backtest, signal_pnl: pd.DataFrame, method: str = "mean"
    ) -> pd.DataFrame:
...
class RiskEngine:
    ...
    def calculate_vol_adjusted_index_from_prices(self, prices_df: pd.DataFrame, br: BacktestRequest) -> pd.DataFrame:
        ...
        returns_df, _leverage_df = self.calculate_vol_adjusted_returns(prices_df, br, returns=False)
    ...
    def calculate_leverage_factor(
        ...
        roll_vol_df = calculations.rolling_volatility(
            returns_df, periods=vol_periods, obs_in_year=vol_obs_in_year
        ).shift(period_shift)
    ...
    def calculate_position_clip_adjustment(
        self, portfolio_net_exposure: pd.DataFrame, portfolio_total_exposure: pd.DataFrame, br
    ) -> pd.DataFrame:
        """Calculate the leverage adjustment to keep net or absolute portfolio exposure within predefined limits.
        ...
        if br.position_clip_rebalance_freq is not None:
            ...
            position_clip_adjustment = filter.resample_time_series_frequency(
                position_clip_adjustment, br.position_clip_rebalance_freq, br.position_clip_resample_type
            )

            _a, position_clip_adjustment = calculations.join_left_fill_right(
