...
class EventStudy(object):
    ...
    def get_economic_event_ret_over_custom_event_day(self,
        ...
        event_dates_nyc = timezone.convert_index_from_UTC_to_new_york_time(
            event_dates)
        ...
        # implied expires on next day eg. 3rd Jan implied ON is 3rd-4th Jan
        # implied

        # TODO smarter way of adjusting dates, as sometimes events can be
        # before/after 10am NY cut
        ...
        # set as New York time and select only those ON vols at the 10am NY cut
    ...
    def get_surprise_against_intraday_moves_over_custom_event(
        ...
        data_frame_agg = None
        # resample to minute freq - in case there are missing values
        data_frame_cross_orig = data_frame_cross_orig.resample('T').mean()

        ef_time_start = ef_time_frame[ticker] - \
            timedelta(minutes=1)  # start time
        indices_start = data_frame_cross_orig.index.isin(ef_time_start)

        for offset in offset_list:
            data_frame_cross = data_frame_cross_orig

            ef_time = ef_time_frame[ticker] + \
                timedelta(minutes=offset - 1)  # end time
            ...
            if data_frame_agg is None:
                ...
                data_frame_agg = data_frame_agg.join(data_frame,
                                                     on=ticker,
                                                     how="inner")
...
class EventsFactory(EventStudy):
    ...
    def load_economic_events(self):
        self._econ_data_frame = self.speed_cache.get_dataframe(
        ...
        if self._econ_data_frame is None:
            ...
            self.speed_cache.put_dataframe(self._db_database_econ_file,
    ...
    def get_economic_event_date_time_fields(self, fields, name, event=None):
        ...
        # construct tickers of the form USD-US Employees on Nonfarm Payrolls
        # Total MoM Net Change SA.actual-release
        for i in range(0, len(fields)):
            ticker.append(
                self.create_event_desciptor_field(name, event, fields[i]))

        # index on the release-dt field eg. 20101230 (we shall convert this
        # later)
        ...
        if ticker_index in self._offset_events:
            data_frame.index = data_frame.index + \
                timedelta(days=self._offset_events[ticker_index])
    ...
    def get_economic_event_vol_over_event_day(self,
        ...
        # return super(EventsFactory,
        # self).get_economic_event_ret_over_event_day(vol_in, name, event,
        # start, end, lagged = realised)
...
class HistEconDataFactory(object):
    ...
    def get_economic_data_history(self,
        ...
        md_request = MarketDataRequest(
            ...
            vendor_fields=
            vendor_fields,  # which Bloomberg/data vendor fields to download
