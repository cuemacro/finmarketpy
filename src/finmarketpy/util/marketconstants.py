"""Module containing constants and configuration values for the finmarketpy project."""
...
class MarketConstants:
    ...
    multiprocessing_library = (
        "multiprocess"  # 'multiprocessing_on_dill' or 'multiprocess' or 'multiprocessing' or 'pathos'
    )
    ...
    backtest_thread_no: ClassVar[dict] = {"linux": 8, "windows": 1, "mac": 8}
    ...
    db_password = "TOFILL"  # noqa: S105

    write_engine = "arctic"

    spot_depo_tenor = "ON"

    currencies_with_365_basis: ClassVar[list] = ["AUD", "CAD", "GBP", "NZD"]

    # Whether to output additional fields related to calculation of total return indices
    output_calculation_fields = False

    ### FX Forwards ###########################################################
    fx_forwards_points_divisor_1: ClassVar[list] = ["IDR"]
    fx_forwards_points_divisor_100: ClassVar[list] = ["JPY"]
    fx_forwards_points_divisor_1000: ClassVar[list] = []

    # Forwards typically used for interpolation (note: eg. TN and SN are swaps)
    fx_forwards_tenor_for_interpolation: ClassVar[list] = [
        "1W",
        "2W",
        "3W",
        "1M",
    ...
    ### FX Options ############################################################
    fx_options_points_divisor_100: ClassVar[list] = ["JPY"]
    fx_options_points_divisor_1000: ClassVar[list] = []

    # Option tenors typically used for interpolation (note: eg. TN and SN are swaps)
    fx_options_tenor_for_interpolation: ClassVar[list] = [
        "ON",
        "1W",
        "2W",
        "3W",
    ...
    # 'nelmer-mead' or 'nelmer-mead-numba' (faster but less accurate) or
    # 'cg' (conjugate gradient tends to be slower, but more accurate)
    fx_options_solver = "nelmer-mead-numba"
    fx_options_pricing_engine = "financepy"  # 'financepy' or 'finmarketpy'

    fx_options_tol = 1e-8

    override_fields: ClassVar[dict] = {}

    # Overwrite field variables with those listed in MarketCred or we can pass through an override_fields dictionary
    def __init__(self, override_fields=None):
        """Initialise MarketConstants, optionally overriding fields from MarketCred or a supplied dictionary."""
