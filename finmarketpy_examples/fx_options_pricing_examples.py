...
from finmarketpy.curve.rates.fxforwardspricer import FXForwardsPricer  # noqa: E402
from finmarketpy.curve.volatility.fxoptionspricer import FXOptionsPricer  # noqa: E402
...
if __name__ == "__main__":
    ...
    if run_example == 1 or run_example == 0:
        ...
        print(
            fx_op.price_instrument(
                cross, pd.Timestamp(horizon_date), "atm", contract_type="european-call", tenor="1M"
            ).to_string()
        )
        ...
        print(
            fx_op.price_instrument(
                cross, pd.Timestamp(horizon_date), "25d-otm", contract_type="european-put", tenor="1W"
            ).to_string()
        )
        ...
        print(
            fx_op.price_instrument(
                cross,
                pd.Timestamp(horizon_date),
                1.50,
                expiry_date=pd.Timestamp(horizon_date) + pd.Timedelta(days=12),
                contract_type="european-call",
            ).to_string()
        )

    ###### Fetch market data for pricing USDJPY FX options over Brexit vote (ie. FX spot, FX forwards, FX deposits and FX vol quotes)  # noqa: E501
    ...
    if run_example == 2 or run_example == 0:
        ...
        print(
            fx_op.price_instrument(
                cross, horizon_date, "atm", contract_type="european-put", tenor="1W", depo_tenor="1W"
            ).to_string()
        )
        ...
        print(
            fx_op.price_instrument(
                cross, horizon_date, "25d-otm", contract_type="european-call", tenor="3M", depo_tenor="3M"
            ).to_string()
        )
        ...
        print(
            fx_op.price_instrument(
                cross, horizon_date, "10d-otm", contract_type="european-put", tenor="1M", depo_tenor="1M"
            ).to_string()
        )
    ...
    if run_example == 3 or run_example == 0:
        ...
        print(
            fx_op.price_instrument(
                ...
                pd.Timestamp(horizon_date),
                0.8124,
                expiry_date=pd.Timestamp(horizon_date) + pd.Timedelta(days=15),
                contract_type="european-call",
            ).to_string()
        )
    ...
    if run_example == 5 or run_example == 0:
        ...
        print(
            fx_op.price_instrument(
                cross, pd.Timestamp(horizon_date), "atm", tenor="1W", depo_tenor="1W", contract_type="european-call"
            ).to_string()
        )
    ...
    if run_example == 6 or run_example == 0:
        ...
        print(
            fx_op.price_instrument(
                cross, pd.Timestamp(horizon_date), "atm", tenor="1W", depo_tenor="1W", contract_type="european-straddle"
            ).to_string()
        )
    ...
    if run_example == 7 or run_example == 0:
        ...
        implied_depo_df = fx_forwards_price.calculate_implied_depo(
            cross, non_usd, market_df=df, fx_forwards_tenor=["1W", "1M"], depo_tenor=["1W", "1M"]
        )
        ...
        fx_op = FXOptionsPricer(
            fx_vol_surface=FXVolSurface(market_df=df, asset=cross, atm_method="fwd", depo_tenor="1M")
        )
        ...
        print(
            fx_op.price_instrument(
                cross, pd.Timestamp(horizon_date), "atm", contract_type="european-put", tenor="1M"
            ).to_string()
        )

        # TODO: calendar around election results in slightly different pricing
        # print(fx_op.price_instrument(cross, pd.Timestamp(horizon_date), "25d-otm", contract_type="european-put", tenor="1W").to_string())  # noqa: E501
        # print(fx_op.price_instrument(cross, pd.Timestamp(horizon_date), 3.5724, contract_type="european-put", expiry_date=pd.Timestamp("2 Nov 2018")).to_string())  # noqa: E501
