"""Module for comparing multiple backtested trading models side by side."""
...
class BacktestComparison:
    """Compares multiple TradingModel backtests by plotting PnL, Sharpe, and trade notionals."""
    ...
    def __init__(self, models, ref_index=0, labels=None):
        """Initialise the comparison with a list of TradingModel instances.

        Args:
            models: Iterable of TradingModel instances to compare.
            ref_index: Index of the reference model in the list (for difference calculations).
            labels: Optional list of labels for the models.
        """
        if hasattr(models, "__iter__") and all(isinstance(x, TradingModel) for x in models):
            self.models = models
            self.ref_index = ref_index
        else:
            msg = "Models need to be an iterable of TradingModel instances."
            raise AttributeError(msg)

        self.labels = labels

    def plot_pnl(self, diff=True, silent_plot=False, reduce_plot=True):
        """Plot the PnL of all models, optionally showing differences from the reference model."""
    ...
    def plot_strategy_trade_notional(self, diff=True, silent_plot=False, reduce_plot=True):
        """Plot the strategy trade notional of all models, optionally showing differences."""
