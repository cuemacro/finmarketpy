...
class Seasonality(object):
    ...
    def time_of_day_seasonality(self, data_frame, years=False, seconds=False):
        ...
        if years is False:
            if seconds:
                return calculations.average_by_hour_min_sec_of_day_pretty_output(
                    data_frame)
            else:
                return calculations.average_by_hour_min_of_day_pretty_output(
                    data_frame)
        ...
        for i in year:
            ...
            temp_seasonality.columns = commonman.postfix_list(
                temp_seasonality.columns.values, " " + str(i))

            if intraday_seasonality is None:
                intraday_seasonality = temp_seasonality
            else:
                intraday_seasonality = intraday_seasonality.join(
                    temp_seasonality)
    ...
    def monthly_seasonality_from_prices(self, data_frame, cum=True,
                                        add_average=False):
        return self.monthly_seasonality(data_frame, cum, add_average,
                                        price_index=True)
    ...
    def adjust_rolling_seasonality(self, data_frame, window=None,
                                   likely_period=None):
        ...
        for c in data_frame.columns:
            data_frame[c] = data_frame[c].rolling(center=False, window=window,
                                                  min_periods=window).apply(
                lambda x: self._remove_seasonality(x,
    ...
    def _remove_seasonality(self, series, likely_period=None):
        ...
        adjusted = adjust_seasons(series, seasons=seasons,
                                  period=likely_period)
