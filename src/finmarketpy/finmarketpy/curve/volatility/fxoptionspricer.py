    ...
    def price_instrument(
        ...
        def _price_option(contract_type_, contract_type_fin_):
            for i in range(len(expiry_date)):
                ...
                # If an implied vol hasn't been provided, interpolate that one, fit the vol surface (if hasn't already been  # noqa: E501
                # done)
                if np.isnan(vol[i]):
                    if tenor is None:
                ...
                if contract_type == "european-call":
                    ...
                    intrinsic_values[i] = max(spot[i] - strike[i], 0) + max(strike[i] - spot[i], 0)
